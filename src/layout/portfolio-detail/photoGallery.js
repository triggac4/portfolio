    ...
    moveBack = (position) => {
        ...
        setTimeout(() => {
            this.setState((state, props) => {
                return {
                    position: 'middle',
                    leftListPosition: reset
                        ? imageLength
                        : state.leftListPosition - 1,
                    middleListPosition: state.leftListPosition,
                    rightListPosition: reset
                        ? imageLength
                        : state.middleListPosition,
                    transition: false,
                }
            })
        }, 200)
        ...
        return (
            <div className='photo-gallery'>
                <div className='photo-gallery__photo'>
                    <div
                        className={`photo-gallery__photo-left ${moveRight}`}
                        style={
                            this.state.transition ? {} : { transition: 'none' }
                        }
                    >
                        <img
                            className='photo-gallery__img'
                            src={imageUrl[this.state.leftListPosition]}
                            alt='1'
                        />
                    </div>
                    <div
                        className={`photo-gallery__photo-middle ${middleMove}`}
                        style={
                            this.state.transition ? {} : { transition: 'none' }
                        }
                    >
                        <img
                            className='photo-gallery__img'
                            src={imageUrl[this.state.middleListPosition]}
                            alt={this.state.middleListPosition}
                        />
                    </div>
                    <div
                        className={`photo-gallery__photo-right ${moveLeft}`}
                        style={
                            this.state.transition ? {} : { transition: 'none' }
                        }
                    >
                        <img
                            className='photo-gallery__img'
                            src={imageUrl[this.state.rightListPosition]}
                            alt='3'
                        />
                    </div>
                </div>
                <button
                    className='photo-gallery__btn-forward'
                    onClick={this.moveForward}
                >
                    &#60;
                </button>
                <button
                    className='photo-gallery__btn-back'
